...
// Modify MenuWindowProc to create solid colored buttons
        ...
        case WM_DRAWITEM:
            if (wParam == 0 || wParam == 1) {  // Button IDs
                ...
                if (lpDrawItem->itemState & ODS_SELECTED) {
                    hBrush = CreateSolidBrush(RGB(200, 200, 200));  // Pressed state
                } else {
                    hBrush = CreateSolidBrush(RGB(240, 240, 240));  // Normal state
                }
    ...
    switch (uMsg) {
        ...
        case WM_PAINT: {
            ...
            if (!g_frameQueue.empty() && g_currentFrameIndex < g_frameQueue.size()) {
                ...
                topGraphics.DrawImage(frame.image, 0, 0);
                
                // Draw queue status on top layer
                Gdiplus::SolidBrush whiteBrush(Gdiplus::Color(255, 255, 255));
                Gdiplus::Font font(L"Arial", 12);
                Gdiplus::StringFormat format;
                format.SetAlignment(Gdiplus::StringAlignmentCenter);
                
                std::wstring status = L"Frame: " + std::to_wstring(g_currentFrameIndex + 1) + 
                                     L"/" + std::to_wstring(g_frameQueue.size()) + 
                                     L" | Delay: " + std::to_wstring(frame.delay) + L"ms";
                
                Gdiplus::RectF statusRect(0, frame.image->GetHeight() - 30, 
                                         static_cast<Gdiplus::REAL>(frame.image->GetWidth()), 30);
                topGraphics.DrawString(status.c_str(), -1, &font, statusRect, &format, &whiteBrush);
...
// Modify QueueFramesFromGif to improve frame management
void QueueFramesFromGif(size_t gifIndex) {
...
// Modify UpdateAppState to set needsClear
void UpdateAppState() {
    if (g_gifs.empty()) {
        return;
    }
    ...
    // Create state change text
    std::wstring stateText;
    
    switch (nextState) {
        case 0:
            g_appState = STATE_MOVE;
            stateText = L"State: MOVE";
        ...
        case 1:
            g_appState = STATE_WAIT;
            stateText = L"State: WAIT";
            break;
            
        case 2:
            g_appState = STATE_SIT;
            stateText = L"State: SIT";
            break;
            
        case 3:
            g_appState = STATE_MISC;
            stateText = L"State: MISC";
    ...
    if (foundGif && newGifIndex < g_gifs.size()) {
        ...
        // Draw text at the top of the window
        Gdiplus::RectF textRect(0, 10, 
                               static_cast<Gdiplus::REAL>(g_gifs[newGifIndex].animation.image->GetWidth()), 30);
        graphics.DrawString(stateText.c_str(), -1, &font, textRect, &format, &whiteBrush);
...
// Modify MoveWindow to reduce unnecessary updates
void MoveWindow() {
    if (g_appState != STATE_MOVE) {
        return;
    }
    
    static int moveCounter = 0;  // Add counter to reduce update frequency
    moveCounter++;
    
    // Only update position every 4 frames to reduce stuttering
    if (moveCounter % 4 != 0) {
        return;
    }
    ...
    if (g_moveDirectionRight) {
        newX += MOVE_DISTANCE * 4;  // Multiply by 4 since we're updating less frequently
    ...
    // Update the window position without forcing a redraw
    SetWindowPos(g_hwnd, NULL, newX, windowRect.top, 0, 0, 
                SWP_NOSIZE | SWP_NOZORDER | SWP_NOREDRAW);
    
    // Only force redraw every 8 frames
    if (moveCounter % 8 == 0) {
        InvalidateRect(g_hwnd, NULL, TRUE);
    }
}

// Modify ToggleMenu to handle menu window visibility
void ToggleMenu() {
